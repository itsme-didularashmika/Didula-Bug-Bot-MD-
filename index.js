...
if (pairingCode && !NanoBotz.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +94771820962")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Silakan Masukan Nomer WhatsAppa nya 🐦\nFor example: +94771820962 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
welcome(iswel, isLeft, NanoBotz, anu)
    ...
    async function getMessage(key){
        ...
        return {
            conversation: "Didula  Bot Ada Di Sini"
        }
    }
